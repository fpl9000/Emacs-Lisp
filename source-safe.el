...
;;; Copyright (C) 1996, 1997, 1998, 1999, 2000 by Rational Software.  All rights reserved.
;;;

(defconst ss-timestamp "Time-stamp: <2013-02-24 12:03:29 puppy>")
...
;;;      - after history hooks
;;;     David Saff <dsaff@tvisions.com>
;;;     Torben Bang Nielsen <tbn@dator.dk>
;;;     Benoit Goudreault <benoit_goudreault@silanis.com>
;;;      - misc bug fixes
...
(defvar ss-update-email-subject "Check-in: >>subject<<"
...
(defvar ss-after-history-hooks nil
  "Hooks to call when a SourceSafe HISTORY command has completed.
When the hooks are called the current buffer contains the history
that has just been checked in and point is at the top.  Useful
to cleanup history unwanted history entries")
...
(defun ss-quote-arg (a)
  (if (string-match " " a)
      (concat "\"" a "\"")
    a))

(defun ss-insert-quoted-arg (a)
  (insert (ss-quote-arg a)))

(defun ss-O@-arg (fname)
  (ss-quote-arg (concat "-O@" fname)))
...
(defun ss-do-command-1 (infile &rest args)
  ...
  (let ((buf (ss-log-buffer))
    ...
    (save-excursion
	  ...
	  ;; UNC name -- too bad
	  nil
	;; With a drive, so easy to cd to it
	...
	(if db
	    (progn
              (insert "set ")
              (ss-insert-quoted-arg (concat "SSDIR=" (ss-external-filename db)))
	      (insert "\n"))))
	...
	(while l
      ...
      (cond ((not has-o-option)
	     ...
	     (insert " " (ss-O@-arg (ss-external-filename outfile)))))
...
(defun ss-do-diff-files (basefile newfile diff-program)
  (let ((buf (get-buffer-create
	      (concat "*DIFF " (ss-file-basename newfile) "*"))))
    (diff basefile newfile (car diff-program))))
  ...
  (cond (interactivep
	...
	((eq (car ss-diff-program) 'SS)
	 (let ((tmpfile (ss-tmp-file "ssdiff.txt"))
	   ...
	   (apply 'ss-do-command "DIFF" f nil
		  (ss-O@-arg (ss-external-filename tmpfile))
...
(defun ss-convert-checkout-to-branch (f status)
  ...
  (let ((tmp (concat f "@TMP"))
    ...
    (if (and (file-exists-p baseline)
	     (not (yes-or-no-p (format "Baseline file %s already exists.  Delete it and continue? "
                                       baseline))))
...
(defun ss-history ()
  ...
  (let ((f (buffer-file-name))
	...
	(progn
	  ...
	  (insert-file-contents tmpfile)
	  (goto-char (point-min))
          ;; Delete any history information (like label events)
          ;; that predates the creation of the file
          (save-excursion
            (if (and (re-search-forward "\\*\nUser:[^\n]*\nCreated" nil t)
                     (search-forward "*******" nil t))
                (delete-region (match-beginning 0) (point-max))))
          ;; All done.  Run hooks.
	  (run-hooks 'ss-after-history-hooks)
...
(defun ss-locate (name)
  ...
  (let ((tmpfile (ss-tmp-file "ssloc.txt")))
    ...
    (ss-do-command "LOCATE" name nil (ss-O@-arg tmpfile))
    (if (file-exists-p tmpfile)
	(progn
...
(defvar ss-maintainer-address "lanning@pobox.com")
...
There are a number of variables of interest:
  ...
  ss-project-dirs
      ...
      Note: You can use either the backward slash \\ or the forward
